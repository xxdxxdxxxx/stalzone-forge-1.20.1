...
public final class ZoneMapScreen extends Screen {
    ...
    public ZoneMapScreen(boolean respawnMode) {
        super(Component.literal("ZoneWars Tactical Map"));
    ...
    private void drawMapBase(GuiGraphics graphics, MapRect map, ZoneWarsState.Snapshot snapshot) {
        ...
        if (texture != null && Minecraft.getInstance().getResourceManager().getResource(texture).isPresent()) {
            graphics.blit(texture, map.x(), map.y(), 0.0f, 0.0f, map.width(), map.height(), map.width(), map.height());
            graphics.renderOutline(map.x(), map.y(), map.width(), map.height(), STROKE);
            return;
        }
    ...
    private void drawBases(GuiGraphics graphics, MapRect map, ZoneWarsState.Snapshot snapshot) {
        for (ZoneWarsState.BaseState base : snapshot.bases()) {
            ...
            graphics.renderOutline(x - 15, y - 12, 30, 24, color);
            graphics.drawCenteredString(this.font, "B", x, y - 4, color);
            graphics.drawCenteredString(this.font, base.team(), x, y + 14, color);
        }
    }

    private void drawRespawns(GuiGraphics graphics, MapRect map, ZoneWarsState.Snapshot snapshot) {
        ...
        for (ZoneWarsState.RespawnState respawn : snapshot.respawns()) {
            ...
            graphics.renderOutline(x - radius, y - radius, radius * 2, radius * 2, active ? GREEN : color);
            graphics.drawCenteredString(this.font, respawn.kind().substring(0, 1), x, y - 4, active ? TEXT : color);
            if (!respawn.available()) {
                String timer = respawn.seconds() < 0 ? "--" : respawn.seconds() + "s";
                graphics.drawCenteredString(this.font, timer, x, y + radius + 4, MUTED);
            }
    ...
    private void drawCapturePoints(GuiGraphics graphics, MapRect map, ZoneWarsState.Snapshot snapshot) {
        for (ZoneWarsState.PointState point : snapshot.points()) {
            ...
            graphics.renderOutline(x - 22, y - 18, 44, 36, color);
            graphics.drawCenteredString(this.font, pointLabel(point), x, y - 8, TEXT);
            graphics.fill(x - 16, y + 9, x + 16, y + 12, 0xFF2D333B);
            graphics.fill(x - 16, y + 9, x - 16 + Math.round(32 * point.progress() / 100.0f), y + 12, color);
        }
    }

    private void drawPlayers(GuiGraphics graphics, MapRect map, ZoneWarsState.Snapshot snapshot) {
        for (ZoneWarsState.PlayerState player : snapshot.players()) {
            ...
            graphics.drawCenteredString(this.font, arrow(player.yaw()), x, y - 4, color);
            graphics.renderOutline(x - size / 2, y - size / 2, size, size, 0xAA070A0D);
            if (player.self() || player.squad()) {
                graphics.drawCenteredString(this.font, player.name(), x, y + 9, player.squad() && !player.self() ? GREEN : color);
            }
        }
    }

    private void drawMarkers(GuiGraphics graphics, MapRect map, ZoneWarsState.Snapshot snapshot) {
        for (ZoneWarsState.MarkerState marker : snapshot.markers()) {
            ...
            graphics.drawCenteredString(this.font, markerLabel(marker.type()), x, y - 4, color);
            if (marker.seconds() > 0) {
                graphics.drawCenteredString(this.font, marker.seconds() + "s", x, y + radius + 3, MUTED);
            }
        }
    }

    private void drawHeader(GuiGraphics graphics, ZoneWarsState.Snapshot snapshot) {
        int center = this.width / 2;
        graphics.fill(center - 190, 8, center + 190, 38, PANEL);
        graphics.renderOutline(center - 190, 8, 380, 30, STROKE);
        graphics.drawCenteredString(this.font, "RED " + snapshot.redScore(), center - 126, 18, RED);
        graphics.drawCenteredString(this.font, formatTime(snapshot.seconds()), center, 18, TEXT);
        graphics.drawCenteredString(this.font, snapshot.blueScore() + " BLUE", center + 126, 18, BLUE);
        ...
        for (int i = 0; i < snapshot.points().size(); i++) {
            ...
            graphics.renderOutline(x - 12, 40, 24, 18, pointColor(point));
            graphics.drawCenteredString(this.font, pointLabel(point), x, 45, TEXT);
        }
    }

    private void drawLegend(GuiGraphics graphics, ZoneWarsState.Snapshot snapshot) {
        int x = 16;
        ...
        graphics.fill(x - 6, y - 5, x + this.font.width(text) + 6, y + 11, PANEL);
        graphics.drawString(this.font, text, x, y, MUTED, false);
    }

    private void drawRespawnControls(GuiGraphics graphics, ZoneWarsState.Snapshot snapshot, int mouseX, int mouseY) {
        ...
        graphics.drawString(this.font, "AVAILABLE SPAWNS", panelX + 12, panelY + 12, MUTED, false);
        ...
        for (int index = 0; index < kinds.length; index++) {
            ...
            graphics.renderOutline(x1, y1, w, h, active ? GREEN : available ? 0xFF465661 : 0xFF303940);
            graphics.fill(x1, y1, x1 + 3, y1 + h, active ? GREEN : available ? spawnColor(kind, state == null ? "NONE" : state.team()) : 0xFF4D565D);
            graphics.drawString(this.font, respawnTitle(kind), x1 + 11, y1 + 10, available ? TEXT : 0xFF68737B, false);
            if (active) {
                graphics.drawString(this.font, "SELECTED", x1 + w - 61, y1 + 10, GREEN, false);
            }
            String meta = respawnMeta(state, kind);
            graphics.drawString(this.font, meta, x1 + 11, y1 + 27, available ? MUTED : 0xFF5C666D, false);
        ...
        graphics.renderOutline(buttonX, buttonY, buttonWidth, buttonHeight, buttonStroke);
        graphics.drawCenteredString(this.font, ready ? "DEPLOY" : "SPAWN UNAVAILABLE", buttonX + buttonWidth / 2, buttonY + 8, ready ? TEXT : MUTED);
        graphics.drawCenteredString(this.font, "ENTER  ·  " + selected, buttonX + buttonWidth / 2, buttonY + 22, ready ? 0xFFD8EBF8 : 0xFF68737B);

        graphics.drawString(this.font, "1-3 SELECT  ·  CLICK MAP MARKER", panelX + 12, panelY + panelHeight - 12, 0xFF778995, false);
        graphics.drawString(this.font, "SELECT DEPLOYMENT", 24, 17, TEXT, false);
    ...
    private void fillWorldRect(GuiGraphics graphics, MapRect map, ZoneWarsState.Bounds bounds, int minX, int minZ, int maxX, int maxZ, int color) {
    ...
    private ResourceLocation mapTexture(String value) {
