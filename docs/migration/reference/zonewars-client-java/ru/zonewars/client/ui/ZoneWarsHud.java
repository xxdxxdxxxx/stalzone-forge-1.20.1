...
public final class ZoneWarsHud {
    ...
    public static void register() {
        HudRenderCallback.EVENT.register(ZoneWarsHud::render);
    }
    ...
    private static void drawTopBar(DrawContext graphics, MinecraftClient client, ZoneWarsState.Snapshot snapshot) {
        int center = graphics.getScaledWindowWidth() / 2;
        int yaw = normalizeYaw(client.player.getYaw());
        ...
        graphics.drawBorder(x, y, width, 38, STROKE);
        graphics.fill(center - 1, y + 5, center + 1, y + 33, 0x557A8995);

        graphics.drawCenteredTextWithShadow(client.textRenderer, String.valueOf(snapshot.redScore()), center - 154, y + 14, RED);
        graphics.drawCenteredTextWithShadow(client.textRenderer, "RED", center - 112, y + 14, RED);
        graphics.drawCenteredTextWithShadow(client.textRenderer, formatTime(snapshot.seconds()), center, y + 12, TEXT);
        graphics.drawCenteredTextWithShadow(client.textRenderer, "BLUE", center + 112, y + 14, BLUE);
        graphics.drawCenteredTextWithShadow(client.textRenderer, String.valueOf(snapshot.blueScore()), center + 154, y + 14, BLUE);
        ...
        for (int i = 0; i < count; i++) {
            ...
            graphics.drawBorder(px - 11, py, 22, 21, color);
            graphics.drawCenteredTextWithShadow(client.textRenderer, pointLabel(point), px, py + 5, TEXT);
    ...
    private static int drawSquad(DrawContext graphics, MinecraftClient client, ZoneWarsState.Snapshot snapshot) {
        int x = 16;
        int y = 24;
        int row = 0;
        for (ZoneWarsState.PlayerState player : snapshot.players()) {
            ...
            if (row == 0) {
                ...
                graphics.drawBorder(x, y, 196, 17, STROKE);
                graphics.drawText(client.textRenderer, "SQUAD", x + 7, y + 5, MUTED, false);
            }
            graphics.fill(x, top, x + 196, top + 17, PANEL_SOFT);
            int color = player.self() ? TEXT : GREEN;
            graphics.drawText(client.textRenderer, trim(player.name(), 16), x + 7, top + 5, color, false);
            int barX = x + 133;
            int barY = top + 7;
    ...
    private static void drawKillFeed(DrawContext graphics, MinecraftClient client, ZoneWarsState.Snapshot snapshot, int startY) {
        int row = 0;
        for (ZoneWarsState.KillFeedState entry : snapshot.killFeed()) {
            int y = startY + row * 17;
            graphics.fill(16, y, 246, y + 15, PANEL_SOFT);
            graphics.drawText(client.textRenderer, trim(entry.killer(), 12), 23, y + 4, teamColor(entry.killerTeam()), false);
            graphics.drawCenteredTextWithShadow(client.textRenderer, ">", 112, y + 4, MUTED);
            graphics.drawText(client.textRenderer, trim(entry.weapon(), 9), 122, y + 4, 0xFFC8D1D8, false);
            graphics.drawText(client.textRenderer, trim(entry.victim(), 12), 184, y + 4, teamColor(entry.victimTeam()), false);
            row++;
        }
    }

    private static void drawMiniMap(DrawContext graphics, MinecraftClient client, ZoneWarsState.Snapshot snapshot) {
        int mapSize = 148;
        int panelWidth = mapSize + 16;
        int panelHeight = mapSize + 30;
        int panelX = graphics.getScaledWindowWidth() - panelWidth - 16;
        int panelY = 16;
        int mapX = panelX + 8;
        int mapY = panelY + 22;
        ...
        graphics.drawText(client.textRenderer, "MINIMAP", panelX + 8, panelY + 7, MUTED, false);
        graphics.drawText(client.textRenderer, "N", panelX + panelWidth - 14, panelY + 7, TEXT, false);
        drawMiniMapBase(graphics, snapshot, mapX, mapY, mapSize);
        graphics.drawBorder(mapX, mapY, mapSize, mapSize, 0xCC9AABB7);
        ...
        for (ZoneWarsState.PointState point : snapshot.points()) {
            ...
            graphics.drawBorder(px - 6, py - 6, 12, 12, color);
            graphics.drawCenteredTextWithShadow(client.textRenderer, pointLabel(point), px, py - 4, TEXT);
        }
        ...
        for (ZoneWarsState.RespawnState respawn : snapshot.respawns()) {
            ...
            graphics.drawBorder(px - 3, py - 3, 7, 7, color);
        }
        ...
        for (ZoneWarsState.MarkerState marker : snapshot.markers()) {
            ...
            graphics.drawBorder(px - 5, py - 5, 10, 10, color);
            graphics.drawCenteredTextWithShadow(client.textRenderer, markerLabel(marker.type()), px, py - 4, color);
        }
        ...
        if (self != null) {
            ...
            graphics.fill(mapX + 1, mapY + mapSize - 13, mapX + client.textRenderer.getWidth(coords) + 7, mapY + mapSize - 1, 0xB20B1117);
            graphics.drawText(client.textRenderer, coords, mapX + 4, mapY + mapSize - 11, MUTED, false);
        }
    }

    private static void drawMiniMapBase(DrawContext graphics, ZoneWarsState.Snapshot snapshot, int x, int y, int size) {
        Identifier texture = mapTexture(snapshot.mapTexture());
        MinecraftClient client = MinecraftClient.getInstance();
        if (texture != null && client.getResourceManager().getResource(texture).isPresent()) {
            graphics.drawTexture(texture, x, y, 0.0f, 0.0f, size, size, size, size);
        } else {
    ...
    private static void fillMiniWorldRect(DrawContext graphics, int originX, int originY, int size, ZoneWarsState.Bounds bounds, int minX, int minZ, int maxX, int maxZ, int color) {
    ...
    private static void drawPlayerMarker(DrawContext graphics, int x, int y, int yaw, int color, boolean self) {
    ...
    private static void drawCompass(DrawContext graphics, MinecraftClient client, int center, int yaw) {
        int width = Math.min(440, Math.max(260, graphics.getScaledWindowWidth() - 620));
        int y = 4;
        graphics.drawCenteredTextWithShadow(client.textRenderer, yaw + "°", center, y, TEXT);
        ...
        for (int offset = -120; offset <= 120; offset += 15) {
            ...
            if (offset % 45 == 0) {
                graphics.drawCenteredTextWithShadow(client.textRenderer, direction(heading), x, y + 24, MUTED);
            }
        }
    }

    private static void drawHitMarker(DrawContext graphics, MinecraftClient client, ZoneWarsState.Snapshot snapshot) {
        ...
        graphics.drawCenteredTextWithShadow(client.textRenderer, activeHit.kill() ? "KILL" : "+" + activeHit.damage(), centerX, centerY + 18, color);
    }
    ...
    private static Identifier mapTexture(String value) {
